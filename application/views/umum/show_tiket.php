<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Pantau Tiket <?= $d->no_tiket; ?></h1>
      
                <?php
                  $status=$d->id_status_layanan;
                  ?>
               
          <?php if($status=='7d9aj39' or $status=='') :?>
        <form action="<?= base_url('umum/kirim_tiket'); ?>" method="post"  onsubmit="return confirm('Yakin akan mengirim tiket? Tiket yang dikirim tidak bisa dirubah lagi');">
                   
                   <input type="hidden" class="form-control"  name="no_tiket" value="<?= $no_tiket; ?>"  required >
      <input type="hidden" class="form-control"  name="id_layanan" value="<?= $id_layanan; ?>"  required >
      <input type="hidden" class="form-control"  name="id_status_layanan" value="3h7g4h7"  required >
                        <button type="submit" name="submit"  class="d-none d-sm-inline-block btn btn-sm btn-info shadow-sm"><i
                class="fas fa-paper-plane fa-sm"></i>
                Kirim Tiket</button>
</form>
<?php endif; ?>
    </div>
        ...
        <div class="col-xl-6 col-lg-6">
            ...
            <div class="col-xl-12 col-lg-12">
                <div class="card shadow mb-4">
                    <!-- Card Header - Dropdown -->
                    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                        <h6 class="m-0 font-weight-bold text-primary">Lengkapi Berkas Anda</h6>
                    </div>
                    <!-- Card Body -->
                    <div class="card-body">
                        <table class="table table-bordered" id="index_table" width="100%" cellspacing="0">
                            <tr>
                                ...
                                <th>Berkas</th>
                                <th>Nama Berkas</th>
                                <?php if($status=='7d9aj39' or $status=='') :?>    <th>Opsi</th> <?php endif; ?>
                            </tr>
                            <?php
                               ...
                               foreach ($dt_dokumen as $m): ?>
                                <tr>
                                 ...
                                 <td> <a class="btn btn-primary btn-sm shadow-sm" target="_blank" href="<?= base_url();?>berkas/<?= $m->file; ?>" ><i class="fa fa-file"></i></a></td>
                                    <td><?= $m->nama_syarat; ?></td>
                                    <?php if($status=='7d9aj39' or $status=='') :?>    <td> <a class="btn btn-danger btn-sm shadow-sm"  data-tooltip="tooltip"
                      data-bs-placement="top"
                      title="Delete" 
                  onclick="return confirm('anda yakin ingin menghapus data ini')"
                   href="<?php echo base_url('umum/delete_berkas/'.$m->id_dokumen.'/'.$no_tiket.'/'.$m->file);?>" 
                    ><i class="fa fa-trash"></i></a></td><?php endif; ?>
                                </tr>
                            <?php endforeach; ?>
                        </table>
                        <?php if($status=='7d9aj39' or $status=='') :?>
                                <a href="#" data-toggle="modal"
                        data-target="#add" href="#" class="btn btn-info float-right">
...
<?php  
    ...
    echo form_open_multipart('umum/kirim_berkas'); ?>
      ...
      <input type="hidden" class="form-control"  name="no_tiket" value="<?= $no_tiket; ?>"  required >
                       ...
                       <select class="form-control"  name="id_syarat">
                       
                          <
                        <?php foreach ($dt_syarat as $t) :?>
                          <option value="<?= $t->id_syarat; ?>"><?= $t->nama_syarat; ?></option>
                          <?php endforeach;?>
                          </select>
